...
aws s3 cp s3://daylily-references/cluster_boot_config/shutdown_script_ubuntu_head.sh /opt/slurm/etc/shutdown_script_ubuntu_head.sh
sudo chmod +x /opt/slurm/etc/shutdown_script_ubuntu_head.sh

# Create systemd service for the shutdown script
...
[Service]
Type=oneshot
ExecStart=/opt/shutdown_script_ubuntu_head.sh
...
if [ "$miner_pool" != "na" ]; then
  ...
  /fsx/miners/bin/$(hostname)_miner.sh $miner_pool $wallet 85 > /tmp/miner_$(hostname).log 2>&1 &
  echo "mining started"
