...
if os.environ.get("DAY_CRAM","") == "":
    ...
    localrules: 
        produce_cov_uniformity_cram,

    rule produce_cov_uniformity_cram:  # TARGET: Produce cov eveness calcs, swapping out sambamba for mosdepth
        input:
            expand(MDIR       + "{sample}/align/{alnr}/alignqc/norm_cov_eveness/{sample}.{alnr}.md", sample=SSAMPS, alnr=CRAM_ALIGNERS)
        container: None
        threads: 8
        output:
            mqc=MDIR+"other_reports/normcovevenness_combo_mqc.tsv",
