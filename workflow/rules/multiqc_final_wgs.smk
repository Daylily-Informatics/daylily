...
rule multiqc_final_wgs:  # TARGET: the big report
    ...
    shell:
        ...
        perl -pi -e "s/REGSUB_SPOTCOST/ per hr(median:\\$dbill$MEDIAN_SPOT_PRICE  mean:$AVERAGE_SPOT_PRICE) per vcpu per min( $VCPU_COST_PER_MIN ) /g;" $(dirname {output})/multiqc_header.yaml;
