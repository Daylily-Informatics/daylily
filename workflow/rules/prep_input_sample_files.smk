...
rule prep_cram_inputs:  # TARGET: Just Pre
    input:
        cram=expand(MDIR + "{sample}/{sample_lane}.cram",sample=SAMPS,sample_lane=SAMPLE_LANES),
        crai=expand(MDIR + "{sample}/{sample_lane}.cram.crai",sample=SAMPS, sample_lane=SAMPLE_LANES)
    output:
        "crams_staged",
    shell:
        "touch  {output}"