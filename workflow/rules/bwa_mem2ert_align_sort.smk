...
rule bwa_mem2ert_aln_sort:
    # bwamem2 branch that uses an enumerated radix tree indexing
    # strategy and gains 30% more speed"""
    ...
    resources:
        ...
        partition=config["bwa_mem2ert_aln_sort"]["partition"],
        vcpu=config["bwa_mem2ert_aln_sort"]["threads"]
    log:
        MDIR + "{sample}/align/bwa2b/logs/align_sort/{sample}.bwa2b.sort.log",
    threads: config["bwa_mem2ert_aln_sort"]["threads"]  # eval( "lambda wildcards,input,threads,  attempt:int( attempt/attempt * config['bwa_mem2ert_aln_sort']['threads'] if attempt < 2 else 254)"),
    benchmark:
        repeat(MDIR + "{sample}/benchmarks/{sample}.bwa2b.alNsort.bench.tsv", 0)
    params:
        cluster_slots=config["bwa_mem2ert_aln_sort"]["threads"], #return_aligner_slot_by_fq_size(MDIR + "{wildcards.sample}/"),
        mdir=MDIR,
        samtmpd="/align/bwa2b/logs/sam_tmpd/",
        bwa_threads=config["bwa_mem2ert_aln_sort"]["bwa_threads"],  # eval( "lambda wildcards, input,threads, attempt: int(attempt/attempt * config['bwa_mem2ert_aln_sort']['other_threads'] if attempt < 2 else 254)"),
        write_threads=config["bwa_mem2ert_aln_sort"]["write_threads"],  # eval( "lambda wildcards, input,threads, attempt: int(attempt/attempt * config['bwa_mem2ert_aln_sort']['write_threads'] if attempt < 2 else 22)"),
        sort_threads=config["bwa_mem2ert_aln_sort"]["sort_threads"],  # eval( "lambda wildcards, input,threads, attempt:  int(attempt/attempt * config['bwa_mem2ert_aln_sort']['sort_threads'] if attempt < 2 else 44)"),
        ...
        lib=" "
        if "lib" not in config["bwa_mem2ert_aln_sort"]
        else config["bwa_mem2ert_aln_sort"]["lib"],
